...
/*===============FIELDS=======================*/
const TCHAR* Field::name() const
{
    return _name;    ///<returns reference
}
const TCHAR* Field::stringValue()
{
    return (valueType & VALUE_STRING) ? static_cast<TCHAR*>(fieldsData) : NULL;    ///<returns reference
}
const ValueArray<uint8_t>* Field::binaryValue()
{
    return (valueType & VALUE_BINARY) ? static_cast<ValueArray<uint8_t>*>(fieldsData) : NULL;    ///<returns reference
}
Reader* Field::readerValue()
{
    return (valueType & VALUE_READER) ? static_cast<Reader*>(fieldsData) : NULL;    ///<returns reference
}
CL_NS(analysis)::TokenStream* Field::tokenStreamValue()
{
...
{
    return (config & INDEX_TOKENIZED) != 0 || (config & INDEX_UNTOKENIZED) != 0;
}
bool    Field::isTokenized() const
{
    return (config & INDEX_TOKENIZED) != 0;
}
bool    Field::isCompressed() const
{
    return (config & STORE_COMPRESS) != 0;
}
bool    Field::isBinary() const
{
    return (valueType & VALUE_BINARY) && fieldsData != NULL;
}

bool    Field::isTermVectorStored() const
{
    return (config & TERMVECTOR_YES) != 0;
}
bool    Field::isStoreOffsetWithTermVector() const
{
    return (config & TERMVECTOR_YES) != 0 && (config & TERMVECTOR_WITH_OFFSETS) != 0;
}
bool    Field::isStorePositionWithTermVector() const
{
    return (config & TERMVECTOR_YES) != 0 && (config & TERMVECTOR_WITH_POSITIONS) != 0;
}

bool Field::getOmitNorms() const
{
    return (config & INDEX_NONORMS) != 0;
}
...
void Field::setBoost(const float_t boost)
{
    this->boost = boost;
}
float_t Field::getBoost() const
{
    return boost;
}
