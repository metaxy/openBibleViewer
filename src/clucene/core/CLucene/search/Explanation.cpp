...
size_t Explanation::getDetailsLength() const
{
...
Explanation* Explanation::getDetail(const size_t i)
{
    return (*details)[i];
}
...
bool ComplexExplanation::getMatch() const
{
    return match;
}
void ComplexExplanation::setMatch(const bool _match)
{
    this->match = _match;
}
bool ComplexExplanation::isMatch() const
{
    return getMatch();
}
